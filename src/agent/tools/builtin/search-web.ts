...
/** 工具定义 */
export const SEARCH_WEB_DEFINITION = {
 ...
 * 模拟搜索结果数据库
 */
const MOCK_WEB_DATA: Record<string, Array<{ title: string; snippet: string; url: string }>> = {
  天气: [
...
/** 工具执行函数 */
export const searchWebExecutor: ToolExecutor = async (args) => {
  const { query } = args as { query: string; limit?: number };
  const limit = (args as { limit?: number }).limit ?? 3;
