...
const agentService = agent_service_js_1.AgentService.getInstance();
/**
 * 注册所有 IPC 处理器
...
function registerIpcHandlers(ipcMain) {
    // ─── Agent 对话 ─────────────────────────────
    ipcMain.handle('agent:ask', async (_event, params) => {
        try {
    ...
    ipcMain.handle('agent:ask-stream', async (event, params) => {
        try {
            if (!params.prompt || typeof params.prompt !== 'string') {
            ...
            await agentService.askStream(params.prompt, params.sessionId, (chunk) => {
    ...
    // ─── 记忆搜索 ───────────────────────────────
    ipcMain.handle('memory:search', async (_event, params) => {
        try {
    ...
    // ─── 配置管理 ───────────────────────────────
    ipcMain.handle('config:get', async (_event, params) => {
        try {
            if (!params.key) {
    ...
    ipcMain.handle('scheduler:add', async (_event, params) => {
        try {
