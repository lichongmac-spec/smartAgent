...
exports.LoopEngine = void 0;
const errors_js_1 = require("../llm/errors.js");
// ============================================================
//  ReAct 系统提示词
// ============================================================
/**
 ...
 */
function buildSystemPrompt(toolsDescription, customPrompt) {
    if (customPrompt) {
...
function buildToolsDescription(tools) {
    ...
    return defs
        .map((t) => {
        const params = Object.entries(t.function.parameters
            .properties ?? {})
...
class LoopEngine {
     ...
     *   });
     */
    constructor(llm, tools, config = {}, onStep) {
        this.llm = llm;
        this.tools = tools;
        this.onStep = onStep;
        this.config = {
    ...
    async run(userInput) {
        ...
        while (this.state.step < this.config.maxSteps) {
            ...
            try {
                ...
                // 智能重试：区分可重试和不可重试错误
                if ((0, errors_js_1.isRetryableError)(error)) {
                    this._retryCount++;
     ...
     */
    async _executeToolCalls(response, messages) {
     ...
     * 将工具观察结果推入消息列表和历史记录
     */
    _pushObservation(messages, observation, toolCall, response) {
    ...
    _trimMessages(messages, maxTokens) {
        ...
        if (available <= 0 || nonSystemMsgs.length <= 2)
            return 0;
