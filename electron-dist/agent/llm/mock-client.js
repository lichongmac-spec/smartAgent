...
const MOCK_TOOL_RULES = [
    {
        ...
        buildArgs: (msg) => {
            // 尝试提取数学表达式
            const mathMatch = msg.match(/(?:计算|算|等于|求(?:值)?)\s*[:：]?\s*([0-9+\-*/().\s×÷^]+)|([0-9]+\s*[+\-*/×÷]\s*[0-9]+)/);
...
class MockLLMClient {
    ...
    constructor() {
        (0, logger_js_1.info)('🟡 Mock 客户端初始化（离线模式 + Function Calling 模拟）');
    }
    /** @inheritdoc */
    async healthCheck() {
    ...
    async chat(messages, options = {}) {
        (0, logger_js_1.debug)(`📤 Mock 请求: ${messages.length} 条消息`);
        // 支持外部 AbortSignal
        ...
        if (!hasToolResults && options.tools && options.tools.length > 0) {
            const mockCall = detectMockToolCall(lastUserMessage, options.tools);
            if (mockCall) {
                (0, logger_js_1.debug)(`📤 Mock 触发工具调用模拟: ${mockCall.toolName}`);
                return {
        ...
        (0, logger_js_1.debug)(`📥 Mock 响应: ${content.slice(0, 80)}...`);
        return {
            content,
            finishReason: 'stop',
            usage: {
    ...
    async *chatStream(messages, options = {}) {
        ...
        const content = this.generateResponse(lastUserMessage, options, messages);
        (0, logger_js_1.debug)(`📤 Mock 流式请求，回复长度: ${content.length} 字符`);
    ...
    //  关键词匹配
    // ============================================================
    generateResponse(message, _options, messages) {
     ...
     */
    generateToolObservationResponse(_userMsg, observation) {
        try {
